...
> 순서: 계획/문서 → 레포 셋업 → CI/CD → PR 자동화 → README → 공통 인프라 코드 → ADR
...
## Phase 2 — 2차 스프린트 (05/28~06/03 오전)

| # | 제목 | 담당 |
|---|------|------|
| #60 | `[ADR] ADR-08 MongoDB UserActivity 업데이트 연결 방식 결정` | 전체 — 기한 **05/28** |
| #61 | `[ADR] ADR-09 S3 백업 파일 포맷 및 경로 구조 결정` | 전체 — 기한 **05/28** |
| #62 | `[CHORE] MongoDB 연동 설정 (이기종 DB 구성)` | 팀장 |
| #63 | `[FEAT] UserActivity 역정규화 모델 설계 및 컬렉션 정의` | 팀장 |
| #64 | `[FEAT] 활동 발생 시 MongoDB 문서 사전 업데이트 훅 연결` | 팀장 |
| #65 | `[REFACTOR] PostgreSQL JOIN 쿼리 → MongoDB 단순 조회 교체` | 팀장 |
| #66 | `[CHORE] AWS S3 연동 설정` | 안준영 |
...
| #70 | `[FEAT] Spring Actuator 커스텀 메트릭 정의` | 김명근 |
| #71 | `[BATCH] 날짜별 로그 파일 S3 업로드 배치 구현` | 노정빈 |

---

## Phase 3 — 중간 발표 준비 (06/03 오후)
